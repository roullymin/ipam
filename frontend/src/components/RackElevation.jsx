...
export default function RackElevation({
  ...
  const labelColumnWidth = Math.max(24, Math.round(unitHeight * 1.45));
  const bodyColumnWidth = Math.max(108, Math.round(unitHeight * 5.8));

  return (
    <div
      className={`mx-auto overflow-hidden rounded-[18px] border border-slate-300 bg-white p-2 shadow-sm ${className}`}
    >
      {showHeader ? (
      ...
      <div
        ...
        style={{
          gridTemplateColumns: `${labelColumnWidth}px ${bodyColumnWidth}px ${labelColumnWidth}px`,
          gridTemplateRows,
        }}
      >
